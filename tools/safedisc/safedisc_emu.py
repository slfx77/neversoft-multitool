...
Same validation strategy used for `ErzDecoder`: run the ROM's own decompressor
under a MIPS interpreter rather than guessing at its output. Here the target is
x86: THUG2.exe's `.text` is encrypted (uniformly high
...
Usage:
    python tools/safedisc/safedisc_emu.py <exe>
    python tools/safedisc/safedisc_emu.py <exe> --max-instructions 50000000
    python tools/safedisc/safedisc_emu.py <exe> --trace 200      # first 200 insns
    python tools/safedisc/safedisc_emu.py <exe> --dump out.bin
...
and 0x7DF81D). That failure and the other partial-dump findings recorded in
`HISTORY.md` are historical; the protected-input decryptor now reconstructs
and validates each corresponding runtime record. See
`docs/backlog/safedisc-emulation-handoff.md` for the exact recovery chain.

Historical development notes have moved to `tools/safedisc/HISTORY.md`. They
remain useful for emulator regressions but are superseded by the status above.
...
# SecServ DLL's own string table (tools/safedisc/safedisc_string_decrypt.py,
...
def main() -> int:
    ...
    ap.add_argument("--dump", type=Path, metavar="OUT.exe",
                    help="Write a loader-readable diagnostic memory image here. "
                         ...
                         "payload bytes; the exact THUG2 build requires "
                         "thug2_safedisc_decrypt.py to authenticate and materialize "
