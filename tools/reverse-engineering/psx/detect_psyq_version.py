...
Requires: ghidra_psx_ldr extension installed under GHIDRA_HOME, or an explicit
          --sigs <signature_dir> argument.
"""
...
def main():
    ...
    sig_dir_default = (
        Path(ghidra_home) / 'Ghidra/Extensions/ghidra_psx_ldr/ghidra_psx_ldr/data/psyq'
        if ghidra_home
        else None
    )
    ...
    if sig_dir is None:
        if sig_dir_default is None:
            print("ERROR: Set GHIDRA_HOME or specify --sigs <signature_dir>")
            sys.exit(1)
