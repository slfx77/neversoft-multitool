...
asserting every Phase-2 Step 5 behavior:
  ...
  gate:
    ...
    - an explicit pending-capture baseline exits 0 with a notice

Usage: python tools/validation/gsdump/native_gate_selftest.py [--root TestOutput/native_gate_selftest]
...
def main():
    ...
    pending_baseline = os.path.join(root, "pending_baseline.json")
    with open(pending_baseline, "w", encoding="utf-8") as f:
        json.dump({"status": "pending-capture", "captures": {}}, f)
