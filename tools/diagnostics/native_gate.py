...
capture against the committed baseline tools/diagnostics/native_baseline.json.
...
Usage:
  python tools/diagnostics/native_gate.py --sweep TestOutput/native_sweep.json
      [--baseline tools/diagnostics/native_baseline.json]
      [--slope-band 0.03] [--mae-tolerance 0.5]

  # (re)write the committed baseline from a sweep JSON (outliers excluded):
  python tools/diagnostics/native_gate.py --sweep TestOutput/native_sweep.json --update-baseline
"""
import json
import os
import sys
...
def run_gate(sweep_path, baseline_path, slope_band, mae_tol):
    ...
    if baseline.get("status") != "active" or not baseline.get("captures"):
        ...
        print(f"  python tools/diagnostics/native_gate.py --sweep {sweep_path} --update-baseline")
        return 0
