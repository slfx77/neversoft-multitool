...
public static class QbKey
{
    ...
    /// </summary>
    public static uint HashLower(string name)
    {
        return Hash(name.ToLowerInvariant());
    }

    public static string? TryResolve(uint hash)
    {
        return KnownNames.GetValueOrDefault(hash);
    }

    public static IReadOnlyDictionary<uint, string> GetAllKnownMappings()
    {
        return KnownNames;
    }
