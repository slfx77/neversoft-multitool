...
internal static class ThawZoneTexHeaderLayoutSupport
{
        ...
        VramUpload? matchedUpload)
    {
        if (selectedBias <= 0 || entry.LayoutMode != 0x02000001)
            return false;
        if (entry.DataSize != 0x2000 || entry.PaletteBytes != 0x20 || entry.DataOffset < 0x138B20 || !matchedUpload.HasValue)
            return false;
        ...
        VramUpload? matchedUpload)
    {
        if (selectedBias <= 0 || entry.LayoutMode != 0x02000001)
            return false;
        if (entry.DataSize != 0x2000 || entry.PaletteBytes != 0x20 || entry.DataOffset < 0x100000 || !matchedUpload.HasValue)
            return false;
