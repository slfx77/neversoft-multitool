...
public static class QbKeyCommand
{
    public static Command Create()
    {
        ...
        command.Subcommands.Add(CreateCrossRefCommand());
        command.Subcommands.Add(CreateImportCommand());
        return command;
    }

    private static Command CreateCrossRefCommand()
    {
    ...
    private static int ExecuteImport(string namesFile, string? psxDir, string? export)
    {
        ...
        if (!string.IsNullOrEmpty(export))
        {
            ...
            ExportMappings(export, allMappings, null);
        }
    ...
    private static PshScanResult RunPshScan(string buildsPath, string psxDir)
    {
        ...
        var stopwatch = Stopwatch.StartNew();
        var scan = QbKeyCrossRef.ScanPshNames(buildsPath, psxDir);
        stopwatch.Stop();
    ...
    private static void ExportMappings(string path, Dictionary<string, uint> crossRefMappings,
