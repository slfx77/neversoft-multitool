...
**Still open, but now with a concrete target (2026-08-16).** The tile's clamp/mirror/wrap state is never parsed, and could not be expressed if it were: `ModelTextureWrap` has only `Repeat` and `ClampToEdge`, so glTF `MIRRORED_REPEAT` is unreachable — though adding the enum value is trivial once we know which textures need it.

Two things were ruled in and one ruled out:

- **The engine really does program wrap per texture.** `G_SETTILE`'s word1 is assembled with variable `cmS`/`cmT` (Spider-Man @`0x800D4CF4`-`0x800D4D18`): `cmT = byte +0x43 & 3` shifted to bits 19:18 and `cmS = byte +0x42 & 3` to bits 9:8, both read from the texture struct at `0x800D4B9C`/`0x800D4BA4`. So mirror and clamp are authored state, not a fixed wrap.
- **Those offsets are a RUNTIME struct, not the dictionary record.** Checked against carved `.tex.n64` bytes: +0x38/+0x3A are not the dimensions and +0x42/+0x43 fall inside pixel data. The next step is therefore bounded and specific — find what writes +0x42/+0x43 of that struct during texture load, and follow it back to the authored field.
- **The group descriptor is not the source.** Its unread u16 at +4..5 is a per-group RDP render-mode/blend selector (bit 0 arms an explicit render-mode emit, bit 1 picks between two blender words), and it is 0x0000 for 15,737 of Spider-Man's 15,769 drawn groups — 25 are 0x0001 and 7 are 0x0003. Far too rare to be a wrap selector.

**Re-check the two reported models before pursuing this**: the half-texel fix may already have resolved the seams, in which case mirror support is a fidelity nicety rather than a bug fix. Two further contributors were identified but deliberately not changed without a measurement: the viewer gives N64 materials linear filtering *with generated mipmaps* (nearest is applied only to PS1 packet materials), and `N64TexFile` keeps the stored RGB of fully transparent texels with no premultiply or edge-dilation pass anywhere, so transparent BLACK darkens every neighbour it is filtered against. Re-check the two reported models before pursuing either.
...
**The torch's fire cannot be yellow, and that is faithful — now proven from the ROM (2026-08-16).** Measured from the carved bank: slot 247 is **14 groups, every one lit and textured**. A lit face's colour is the light alone — the authored bytes are normals — and each ROM uploads exactly one monochrome grey `Lights1`, so the N64 port has no coloured light to give it.

The open doubt was whether the shade should be applied at all, since the display-list token stream carries no combiner state and a self-illuminated surface using a decal combiner would ignore it. **It is a multiplier.** The model draw path emits exactly two combiners, selected by kind bit 0 (Spider-Man @`0x800D2154`, word1 @`0x800D2178`): `0xFC127E05` gives cycle 0 `TEXEL0 * SHADE` for a textured group and `0xFC527E1F` gives `ENVIRONMENT * SHADE` for an untextured one, sharing word1 `0xFFFFF2F8` whose b and d slots are 0. A decal combiner that would ignore shade does exist in these ROMs (`0xFCFFFFFF FFFCF279`) but only in the 2D blitter, which reads no group descriptor.

Two Rosetta measurements agree: the PS1 sibling's authored packet colours average **0.437 against its 0.502 neutral** — near-neutral modulation, not "the authored colour *is* the fire" — and the two ports ship **the same flame art at the same brightness** (mean RGB 126.3,43.4,9.4 vs 126.4,43.4,9.3), so the port did not pre-brighten to survive a dim shade. The brightness was the defect; the absence of hue is the console's own output.

**Residual, recorded in `ComputeN64VertexColour`:** cycle 1 is `COMBINED * ENVIRONMENT + PRIMITIVE` and neither global is exported. Both are BSS (`0x80105084`, `0x80105080`) with no direct-offset store anywhere in the image, so their runtime values are unrecovered. ENVIRONMENT can only darken further; **PRIMITIVE is additive and could brighten**, so it is the one thing that could still make a glowing surface brighter than we export.

**Found while proving it:** the engine has a *second* group skip the decoder was ignoring — `kind & 0x0800`, tested beside the `0x8000` skip in both group loops of all four ROMs. Honouring it drops 14.4% of decoded groups (11,613 of 80,533) but only 0.06–0.63% of *exported* triangles, because most skipped groups belong to nodes no object places. Shipped 2026-08-16.
