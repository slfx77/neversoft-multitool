...
- Export sweep: `python tools/research/thps3-animation/thps3_variant_sweep.py --out TestOutput/thps3_variant_sweep`
...
Historical status from that pass: no PCSX2 process was running, PINE was
disabled in `PCSX2.ini`, and no `SLUS-20013` / `F77E2FB5` THPS3 savestate was
present in the then-current default PCSX2 sstates directory, so live capture
was not completed in that pass.
