...
- **Xbox MDL/SKIN meshes**: Native Xbox/PC scene files (.skin.xbx, .mdl.xbx, .skin.wpc, .mdl.wpc) from THUG2 → glTF (.glb) with vertex colors, normals, UV coordinates, and material references. Version triple (1,1,1). Multi-pass materials (up to 4 passes). THUG2 format: per-mesh interleaved vertex buffers (not per-sector flat arrays like THUG1). Degenerate triangle strip index buffers (u16). Skinning weights: packed u32 (11+11+10 bits). 925 files (751 SKIN + 174 MDL), 502,851 triangles, 0 parse failures, 0 glTF errors. Hierarchy links in MDL files for multi-part objects (vehicles). Format reference: nxtools fmt_thscene_import.py (ground truth) + THUG source material.cpp. CLI: `xbxscene <input> [-o output] [-t] [--tex path] [-v]`.
...
- **Unpack tab** (GUI + CLI): Recursive extraction of all game archives in a directory. Multi-pass loop: scans for archives → extracts → rescans for nested archives (e.g. WAD containing PRE) → repeats until no new archives found. Supports WAD, PRE, compressed PRE (PRE3/PRX), PKR, DDX, BON. Already-extracted detection (skips if `{stem}/` directory exists and is non-empty). GUI: folder picker, archive list with status/progress, cancel support. CLI: `unpack <input-dir> [-v]`.
