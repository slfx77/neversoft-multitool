...
- **THAW (.skin.ps2 meshes)**: THAW uses a different .skin.ps2 binary format than THUG2. Header: numObjects(u32) + totalMeshes(u32×2) + dataSize(u32) + bsphere(16B) + object table(8B×N: checksum+meshCount) + entry table(64B×M) + VIF data. 332 files. No .geom.ps2 files exist. nxtools has NO PS2 importer. PC/Xbox THAW meshes are fully supported (723/723 files via `ThawSceneFile.cs`). Remaining: .skin.ps2 meshes (332 files), .tex.ps2 scene texture metadata (328 files).
