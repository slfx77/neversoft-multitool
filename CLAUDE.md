...
- **THPS4 Skeleton Format (.ske)**: 34 `.ske` files in THPS4 `SKE/` (Sample) or `Skeletons/` (WAD) directory — cross-platform skeleton format distinct from the PS2-specific `.ske.ps2` used by THUG/THUG2. Header starts with a name checksum (e.g. `0x93D09490`), NOT a version number — structurally different from `Ps2SkeletonFile` which expects version(u32) + flags(u32) + numBones(i32). THUG/THUG2 have BOTH `.ske` and `.ske.ps2` for the same models, suggesting `.ske` is the cross-platform format read by `Gfx/Skeleton.cpp` and `.ske.ps2` is the PS2-native format read by `Gfx/NGPS/NX/scene.cpp`. `TryDiscoverSkeleton()` already searches for `.ske` extension via `CompanionSearch`. When implemented: (1) new parser for `.ske` format (currently found but parse fails silently), (2) enables THPS4 skinned mesh export with bone hierarchy (currently converts without skeleton).
