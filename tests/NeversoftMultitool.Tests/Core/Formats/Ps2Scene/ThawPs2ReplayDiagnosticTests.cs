        ...
        foreach (var kick in kicks)
        {
            ...
            if (batch.PostBatchElements.Length > 0)
            {
                ...
                for (var i = 0; i < batch.PostBatchElements.Length; i++)
                {
                    ...
                    if (c0Active)
                        lines.Add($"    C0→C1: src={element.C0 & 0x3FF}(raw=0x{element.C0:X4}) → dst={element.C1 & 0x3FF}(raw=0x{element.C1:X4})");
                    else
                        lines.Add($"    C0→C1: [TAG] raw=0x{element.C0:X4},0x{element.C1:X4}");

                    if (c2Active)
                        lines.Add($"    C2→C3: src={element.C2 & 0x3FF}(raw=0x{element.C2:X4}) → dst={element.C3 & 0x3FF}(raw=0x{element.C3:X4})");
                    else
