...
public sealed class ThawPs2ReplayTriangleDiagnosticTests(TestPaths paths)
{
    ...
    public void Diagnostic_Ps2VsPc_TriangleComparison(string stem, int expectedPcTriangles)
    {
        ...
        foreach (var kick in kicks)
        {
            ...
            lines.Add(
                $"Kick {kick.KickIndex}: setup={kick.SetupIndex} addr={kick.KickPacket.Address} nloop={kick.KickPacket.Nloop}");
            lines.Add(
                $"  gaps={gaps} carries={carries} noKicks={noKicks} tris={kick.TriangleCount} meshes={kick.Meshes.Length}");
            ...
            if (gaps > 0)
            {
                ...
                lines.Add(
                    $"  gap runs: {gapRuns.Count} (sizes: {string.Join(",", gapRuns.Select(run => run.Length))})");
            }
        ...
        lines.Add(
            $"PS2 setup indices used: [{string.Join(",", kicks.Select(kick => kick.SetupIndex).Distinct().OrderBy(index => index))}]");
        lines.Add("");
        ...
        lines.Add("");
        foreach (var position in missingPcPositions.OrderBy(point => point.X).ThenBy(point => point.Y)
                     .ThenBy(point => point.Z).Take(30))
        {
        ...
        lines.Add(
            $"Global best match score <= 0.10: {heuristicMatches.Count(match => match.GlobalMatch.Metrics.Score <= 0.10f)}");
        lines.Add(
            $"Global best match score <= 0.25: {heuristicMatches.Count(match => match.GlobalMatch.Metrics.Score <= 0.25f)}");
